...
* 단계 2: 어드민 예약 내역 관리 화면 출력 및 예약 조회 기능
...
* 단계 5: 예약 내역을 조회하는 기능을 구현함에 있어 데이터베이스를 적용하도록 한다.
...
* 단계 6: 예약 내역을 추가 및 삭제하는 기능을 구현함에 있어 데이터베이스를 적용하도록 한다.
...
* 단계 7: 어드민 시간 관리 화면 출력
    - 시간 관리 조회, 추가 및 취소 API를 구현해 시간 관리 내역이 출력되게 한다.

> 시간 관리 관련 데이터베이스 스키마 추가 (schema.sql)
...
* 단계 8: 어드민 예약 내역과 시간 관리 테이블 연결
    - 예약 내역 페이지에서 시간 테이블에 저장된 값만 선택될 수 있게 수정한다.
    - 예약 클래스의 시간 타입을 String에서 ReservationTime 객체로 수정하고, 기존의 예약 조회, 추가 쿼리를 알맞게 변경한다.

> 예약 내역과 시간 관리 관련 데이터베이스 스키마 변경 (schema.sql)
