...
- `RowMapper` ,`PreparedStatement` , `ResultSet`의 관계와 어떻게 동작하는지, 무슨 필요성에 의해 개발되었고 최신 현재는 주로 어떤걸로 해당 기능을 구현하고있는지. 익숙한
  JPA와 의 관계는 ?
- `KeyHolder` ,`GeneratedKeyHolder` 의 관계와 어떻게 동작하는지, 무슨 필요성에 의해 개발되었고 최신 현재는 주로 어떤걸로 해당 기능을 구현하고있는지, 익숙한 JPA와 의 관계는 ?
    - **`prepareStatement(sql, new String[]{"id"})`**
        ...
        - *"jdbcTemplate.update의 PreparedStatementCreator 오버로드는 정확히 어떻게 동작하나" 더해서 이런 부분까지 학습할지 고려. 과거의 기술이라서 굳이 현재
          동작원리까지 알아야하는 생각이 들김함. 우선순위를 메겨도 좋을듯*
...
- "H2/Spring Boot에서 schema.sql 실행 순서, FK 검사 시점, DROP/CREATE 정책”
    - `reservation`이 `reservation_time`을 참조(FK)한다. SQL 실행 시점에 참조 대상이 이미 존재해야한다. 그래서 **`schema.sql`** 작성시 참조 관계를 고려해야함.
      고려 안하면 FK 제약 생성 실패함.
- 트랜잭션 필요성
...
### 도메인 객체와 id 필드의 관계(엔티티 관련 개념)

<details>
- 현재 도메인이 id를 가지고 있어서 영속화 전엔 null 영속화 후에는 값이 채워지는 두 상태가 같은 클래스에 공존하고있다.(id의 null 검사를 도메인 단위테스트에서 하려고 하다가 찜찜함을 느낌)
- id는 도메인 개념이 아니라 영속성 개념이다.
- JPA의 @Entity는 본질적으로 DB 레코드를 자바 객체로 사용하기 위한 ‘매핑수단?’이다.
- 도메인과 영속화 엔티티를 분리해야 겠다는 필요성을 어렴풋이 느낌
    - JPA의 @Entity 어노테이션이 붙은 클래스와 도메인 객체 클래스의 명확한 차이는 ?
    - 영속화 전후 두상태를 같은 클래스로 표현하는게 정확히 뭐가 문제인가?
    - DDD에서 말하는 Aggregate Root, Entity, Value Object란 ? 어떤 맥락에서 해당 개념들이 나오게 되었고 그로 인해 어떤 부분의 장점을 얻었는가 ?
</details>
...
<details>
- DTO 객체들에서 사용하지 않는 메서드를 지우려고 하는 과정 중 record 문법이 떠올라서 보류
