...
- [ ] 홈 화면
    - [ ] API 경로는 `GET /admin/time` 으로 구현한다.
    - [ ] 메인 페이지 파일은 `templates/admin/time.html`를 사용한다.
    - [ ] 응답 성공시, `200 OK`를 반환한다.
- [ ] 시간 조회 기능
    - [ ] `GET /admin/times` 요청 시 시간 관리 페이지를 응답해야 한다.
    - [ ] 시간 조회 페이지 파일은 `templates/admin/time.html`를 사용한다.
    - [ ] 응답 성공시, `200 OK`를 반환한다.
    - [ ] `GET /times`요청 시 시간 목록 조회 데이터를 응답해야 한다.
    - [ ] 시간 목록 조회 데이터 구조는, `시간 번호`, `시간`,으로 이루어져 있다.
    - [ ] 응답 성공시, `200 OK`를 반환한다.
- [ ] 시간 추가 기능
    - [ ] `POST /times`요청 시 시간 데이터를 추가한다.
    - [ ] 요청 데이터 구조는 `시간` 으로 이루어져 있다.
    - [ ] 응답 성공시, `200 OK`를 반환한다.
    - [ ] 응답 데이터 구조는 `시간 번호`, `시간` 으로 이루어져 있다.
    - [ ] 중복되는 시간일 시 `400 Bad Request`를 반환한다.
- [ ] 시간 취소 기능
    - [ ] `DELETE /times/{id}`요청 시 대상 데이터를 제거한다.
    - [ ] 응답 성공시, `200 OK`를 반환한다.
    - [ ] 존재하지 않는 시간 번호일 시 `404 Not Found`를 반환한다.
