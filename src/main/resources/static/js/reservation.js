...
function render(data) {
    ...
    data.forEach(item => {
        ...
        row.insertCell(1).textContent = item.userName.name;  // 'name' 객체 안의 'name' 값
        row.insertCell(2).textContent = item.reservationDateTime.date.date;  // 'date' 객체 안의 'date' 값
        row.insertCell(3).textContent = item.reservationDateTime.time.startAt;  // 'time' 객체 안의 'start_at' 값
