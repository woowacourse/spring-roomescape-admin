...
public class ReservationGetResponseTest {
    ...
    void Reservation_엔티티로부터_해당_Dto를_파싱한다() {
        ...
        Reservation reservation = new Reservation("프리", LocalDate.of(2025, 4, 24), reservationTimeEntity);
        Reservation reservationEntity = reservation.toEntity(1L);

        // When
        ReservationGetResponse reservationGetResponse = ReservationGetResponse.from(reservationEntity);
        ...
        assertThat(reservationGetResponse.time()).isEqualTo(ReservationTimeGetResponse.from(reservationTimeEntity));
    }
}
