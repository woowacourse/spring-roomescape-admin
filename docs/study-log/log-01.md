...
- `@RestControllerAdvice`: `@ControllerAdvice` + `@ResponseBody`
    - 모든 컨트롤러에서 발생하는 예외를 가로채는 '전역 예외 처리기' 역할
    - 특정 컨트롤러에 종속되지 않으므로 코드 중복이 줄어들고, 응답 형식을 일관되게 유지 가능

- Request에 포함되는 4가지
    - Method (방식): 무엇을 할 것인가? (조회/저장)
    - URL (주소): 어디에 요청할 것인가? (`/reservations`)
    - Headers (헤더): 메타 정보 (나 JSON 데이터 보낼게, 내 브라우저 정보는 이거야 등)
    - Body (바디): 실제 데이터 (예약 날짜, 이름 등)

#### MissionStepTest

---

- POST와 DELETE의 상태 코드를 바꾸니까 테스트도 같이 바꿔줘야 했다.

#### GlobalExceptionHandler

---

- `RuntimeException` 발생 → `@ExceptionHandler(RuntimeException.class)`가 존재하므로 400 에러 반환
- `Exception`(또는 `RuntimeException`이 아닌 다른 예외) 발생 → 더 구체적인 핸들러가 없으므로 가장 포괄적인 `@ExceptionHandler(Exception.class)`가 작동하여 500 에러 반환

## Postman

---

- Request: `예약 목록 조회(GET)`, `예약 목록 추가(POST)` 같이, 특정 URL과 설정을 세팅해둔 하나의 작업 단위
- 상태 코드
    - `200 OK`: 요청이 성공적으로 처리되었다는 일반적인 응답
    - `201 Created`: REST API 규약에서는 새로운 리소스가 생성되었을 때 `201`을 반환하는 것을 권장함
    - `204 No Content`: DELETE 성공 시 본문(Body) 없이 성공 사실만을 알리는 규약을 적용함 ("성공했고, 줄 데이터는 없어"라는 의미 전달용)
- Path Variable: `/reservations/{id}`와 같이 URL 경로에 변수를 담아 특정 자원을 식별함
- 서버를 껐다 켜면 인메모리 저장소의 데이터가 초기화되어서 `ArrayList`에 저장했던 데이터가 모두 날아감 → 새로 POST를 하지 않은 상태에서 DELETE를 요청하면 찾을 대상이 없어서 예외가 발생함
    - 올바른 테스트 순서
        1. 서버를 켬 → 빈 리스트 상태로 시작
        2. 데이터 조회 (GET): 목록을 조회해서 리스트가 비어있는지 확인
        3. 데이터 생성 (POST): `예약 생성` 요청을 보냄
        4. 데이터 조회 (GET): 목록을 조회해서 데이터가 들어갔는지 확인
        5. 데이터 삭제 (DELETE): 확인한 ID를 주소창 뒤에 넣음 (예: `localhost:8080/reservations/1`)
        6. 다시 조회 (GET): 목록을 조회했을 때 빈 배열(`[]`)이 나온다면 성공

#### RuntimeException 발생 시의 응답 구조

- 별도의 예외 처리를 하지 않고 서버에서 `RuntimeException`이 발생하면, 스프링 부트의 `BasicErrorController`가 이를 가로채서 표준적인 에러 응답을 생성함
- `http://localhost:8080/reservations/100` 요청 → 500 Internal Server Error

    ```json
    500 Internal Server Error
    
    {
        "timestamp": "2026-05-02T19:19:04.422+00:00",
        "status": 500,
        "error": "Internal Server Error",
        "path": "/reservations/100"
    }
    ```

  | **구성 요소** | **역할** |
      | --- | --- |
  | **DispatcherServlet** | 요청을 처리하던 중 발생한 예외를 감지함 |
  | **BasicErrorController** | 스프링 부트가 기본으로 제공하는 컨트롤러로, 예외 발생 시 `/error` 경로로 요청을 전달받아 응답을 생성함 |
  | **Default Error Attributes** | `timestamp`, `status`, `error`, `path` 같은 정보들을 자동으로 수집하여 JSON으로 변환함 |
- 예외 메시지 던지는 여부에 따른 예외 결과

    ```
    java.lang.RuntimeException: 의도적인 서버 에러 발생!
    ```
...
**Q1. 서버에서 요청을 처리하다가 예외가 발생하면 어떻게 대응하는게 좋을까?**

1. 클라이언트의 잘못(잘못된 ID, 잘못된 형식)이라면 400번대를, 서버 내부의 로직 결함이나 DB 장애라면 500번대 상태 코드를 반환하여 예외에 대해 명확하게 알려주는 게 좋을 것 같습니다.
2. 에러 형식을 통일해서 전달해야 할 것 같습니다.
3. 사용자에게는 그에 맞는 메시지를 주되, 구체적인 예외 상황을 알려서는 안될 것 같습니다.
4. 클라이언트에게는 추상적인 메시지를 주더라도, 서버 로그에는 개발자가 원인을 파악할 수 있도록 상세한 에러 내용을 남겨야 할 것 같습니다.

**Q2. 서버에서 오류에 대해 어떤 상태코드를 내려주는 것이 제일 좋을까?**

- **4xx**: 클라이언트가 잘못된 데이터를 보냈거나(400), 권한이 없거나(401, 403), 없는 페이지를 찾을 때(404) 사용합니다.
- **5xx**: 서버 로직에 버그가 있거나, DB 연결이 끊기는 등 서버에 문제가 생겼을 때 사용합니다.

**Q3. http 상태코드는 무엇이고, 상태코드 400, 500번대에는 어떤 값들이 있을까?**

| **코드** | **명칭** | **의미** |
| --- | --- | --- |
| **400** | **Bad Request** | 요청 구문이 잘못되었거나 파라미터가 유효하지 않음 |
| **401** | **Unauthorized** | 인증되지 않은 사용자 |
| **403** | **Forbidden** | 인증은 되었으나 해당 자원에 접근 권한이 없음 |
| **404** | **Not Found** | 요청한 자원이 존재하지 않음 |
| **405** | **Method Not Allowed** | 해당 URL에서 지원하지 않는 HTTP 메서드 (예: POST인데 GET으로 보냄) |
| **500** | **Internal Server Error** | 서버 내부의 알 수 없는 오류 (가장 일반적인 서버 에러) |
| **502** | **Bad Gateway** | 게이트웨이 또는 프록시 서버에 오류가 발생함 |
...
- 코드를 조금씩 다듬어가는 방식으로 공부하니까 이해가 수월하게 되는 것 같습니다.
- 포스트맨으로 실제로 request를 만들면서 실습하니 개념이 글로만 공부할 때보다 잘 와닿았습니다.
